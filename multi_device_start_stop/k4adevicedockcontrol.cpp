...
K4ADockControlStatus K4ADeviceDockControl::Show()
{
    ...
    else
    {
        ...
        ImGuiExtensions::ButtonColorChanger dd(ImGuiExtensions::ButtonColor::Red);
        if (ImGui::Button("reset", buttonSize))
        {
            m_device.switch_device_clock_sync_mode(K4A_DEVICE_CLOCK_SYNC_MODE_RESET, 0);
        }

        ImGuiExtensions::ButtonColorChanger ff(ImGuiExtensions::ButtonColor::Green);
        if (ImGui::Button("sync", buttonSize))
        {
            m_device.switch_device_clock_sync_mode(K4A_DEVICE_CLOCK_SYNC_MODE_SYNC, 0);
        }
    }
